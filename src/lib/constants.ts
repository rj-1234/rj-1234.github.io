...
export const METRICS: Metric[] = [
  {
    ...
    sublabel: 'Annual production volume — PII Service',
  ...
  },
  {
    ...
    sublabel: 'Call Intelligence Engine — annual volume',
...
export const PROJECTS: Project[] = [
  {
    ...
    impact: [
      ...
      'Enables downstream AI products (Call Intelligence Engine, LLMs) to operate on live data',
    ],
    architecture: 'Client App → API Gateway → PII Service (EKS) → NER Model (PyTorch) → Redacted Output',
  },
  {
    id: 'deepci',
    title: 'Call Intelligence Engine — Enterprise Speech Analytics',
    ...
    architecture: 'Audio Ingestion → Denoising → ASR → PII Redaction → Diarization → Classification → Insights API',
  },
  {
    id: 'nucleus',
    ...
    architecture: 'Product Teams → Client SDK → Auth (Okta) → Service Router → [LLM | Redaction | Embeddings]',
  },
  {
    id: 'guardian',
    ...
    architecture: 'Request → RAI Orchestrator (FastAPI) → NeMo Rails Engine → [Block | Pass | Flag] → Evaluation Pipeline → Self-Heal (LoRA)',
  },
  {
    id: 'charlie',
...
export const TERMINAL_SNIPPETS: TerminalSnippet[] = [
  ...
  {
    id: 'pii-redaction',
    filename: 'pii_service.py',
    runLabel: 'Run Redact',
    code: [
      'from pii_service import redact',
      '',
  ...
  {
    id: 'guardrails',
    filename: 'rai_check.py',
    runLabel: 'Check Safety',
    code: [
      'from rai import RailsEngine',
      '',
  ...
  },
  {
    ...
    runLabel: 'Search',
    code: [
      'from awesome_sdk import EmbeddingClient',
      '',
